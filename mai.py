...
# Función para obtener el total de citas de un autor
@st.cache_data
def get_total_citations(df, selected_id):
    ...
    return df_filtered["Cited by"].fillna(0).astype(int).sum()

# Función para obtener el total de artículos de un autor
@st.cache_data
def get_total_articles(df, selected_id):
    ...
    return df[df["Author(s) ID"].str.contains(selected_id, na=False, case=False)].shape[0]

# Función para obtener los años de publicación de un autor
@st.cache_data
def get_publication_years(df, selected_id):
    ...
    return years.min(), years.max(), years.value_counts().sort_index()

# Función para obtener información de las editoriales
@st.cache_data
def get_publisher_info(df, selected_id):
    ...
    df_filtered = df[df["Author(s) ID"].str.contains(selected_id, na=False, case=False)]
    return df_filtered.groupby("Publisher").agg(
        ...
        if available_authors:
            ...
            if selected_id:
                ...
                if matching_authors:
                    ...
                    st.write(f"**Total de citas asociadas:** {total_citations}")

                    total_articles = get_total_articles(df, selected_id)
                    st.write(f"**Total de artículos en los que participa:** {total_articles}")

                    min_year, max_year, year_counts = get_publication_years(df, selected_id)
                    if min_year and max_year:
                        st.write(f"**Año más antiguo de publicación:** {min_year}")
                        st.write(f"**Año más reciente de publicación:** {max_year}")
                    ...
                    if not publisher_info.empty:
                        st.write("**Editoriales en las que ha publicado este ID:**")
                        st.dataframe(publisher_info)
                    else:
